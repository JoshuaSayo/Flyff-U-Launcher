...
---
## 版本 4.0.2-automation.9

### 移动怪物的实时重新定位

- 每次接近点击都会使用最新且高于阈值的怪物名称匹配重新计算。
- 第一次点击靠近名称下方；两个小范围左右候选点用于覆盖移动中的小型怪物。
- 未选中的怪物离开扫描区域时，旧坐标会被丢弃，状态机立即恢复搜索。

目标模板和扫描区域说明见 [AUTOMATION_INSTRUCTIONS.md](../../AUTOMATION_INSTRUCTIONS.md)。
...
- 无效的旧死亡模板不再停止战斗；除有限次数的 Support 复活需要外，死亡检测为可选功能。
- 匹配到怪物名称后，现在会转换为点击名称下方的怪物本体。
- 版本 6 配置会安全迁移，并默认关闭可选死亡检测。

更新后的设置步骤请参阅 [AUTOMATION_INSTRUCTIONS.md](../../AUTOMATION_INSTRUCTIONS.md)。
...
- 战斗会先点击匹配到的怪物，确认已选目标 HP，再等待通过结构检测识别到红色准星后才开始攻击。
- 白色选择准星、红色 HP 条和红色文字都不会被当作战斗确认。
- 技能循环为可选项且默认关闭；不配置技能按键也可以使用普通点击攻击。
- 选择和进入战斗最多尝试三次点击，并受接近状态超时限制。

引导式校准步骤请参阅 [AUTOMATION_INSTRUCTIONS.md](../../AUTOMATION_INSTRUCTIONS.md)。
...
- 预览和 FSM 现在直接捕获所选的内嵌 Flyff 游戏画面，不再捕获启动器背景。
- 更新时会一次性清除旧的像素区域和模板；按键与行为设置会保留。
- 新增归一化结构匹配，并阻止过大或细节不足的模板和 HP 区域。

所需的一次性重新校准步骤请参阅 [AUTOMATION_INSTRUCTIONS.md](../../AUTOMATION_INSTRUCTIONS.md)。
