...
2. Lock that label as the active monster, translate it to a close body point, and click.
3. Reacquire only that moving label in a local tracking window and try a small left/right fallback if the red crosshair has not appeared.
4. Enter **ATTACKING** only after the red crosshair is structurally confirmed.

Selection clicks are limited to three per approach attempt. The first click uses the global scan; retries require a fresh above-threshold match inside the active monster's local tracking window. Identical labels elsewhere cannot steal the retry. If the locked label is lost, stale coordinates are cleared and the FSM returns to searching. Target HP remains visible as optional diagnostic telemetry when calibrated.
...
| Approaching | Tracks one chosen label locally and tries a bounded center/left/right body point | Red crosshair confirmation, target lock is lost, or approach timeout |
...
- Moving targets are reacquired before each click inside a local window around the chosen monster. If the lock is lost, its old point is discarded and the FSM resumes searching.
- Inspect `Target click N/3 at (x, y)` in the launcher log. Retries say `locked fresh` and should stay near the initial monster.
